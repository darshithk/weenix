...
sched_cancellable_sleep_on(ktqueue_t *q)
{
        ...
        KASSERT(q != NULL && "Queue to sleep on is empty");
        dbg(DBG_PRINT,"(GRADING1C)\n");
        //If thread is already cancelled, it should return.
        ...
        //When it wakes up from sleep, it is cancelled, thread should exit
        if(curthr->kt_cancelled == 1){
                dbg(DBG_PRINT,"(GRADING1C)\n");
                return -EINTR;
        }
...
sched_cancel(struct kthread *kthr)
{
        ...
        KASSERT(kthr != NULL && "Cancel called on NULL");
        kthr->kt_cancelled = 1;
...
sched_switch(void)
{
        ...
        //Need to handle the case where curthr is NULL due to all threads waiting for something
        //Although this might not be possible with MTP=0

        uint8_t interrupt_level = intr_getipl();
        intr_setipl(IPL_HIGH);

        kthread_t *thread_to_run = NULL;
        dbg(DBG_PRINT,"(GRADING1C)\n");
        ...
        thread_to_run = ktqueue_dequeue(&kt_runq);

        kthread_t *thread_to_be_switched = curthr;
        curthr = thread_to_run;
        curproc = curthr->kt_proc;
        dbg(DBG_PRINT,"(GRADING1C)\n");
        context_switch(&thread_to_be_switched->kt_ctx, &thread_to_run->kt_ctx);
        dbg(DBG_PRINT,"(GRADING1C)\n");
        intr_setipl(interrupt_level);
}
