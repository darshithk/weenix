...
kmutex_lock(kmutex_t *mtx)
{
        ...
        KASSERT(mtx != NULL && "Attempting to lock on NULL");
...
kmutex_lock_cancellable(kmutex_t *mtx)
{
        ...
        if(mtx->km_holder == NULL){
                ...
                // When cancelled, the thread is made runnable by sched_cancel
                int sched_status;
                while(mtx->km_holder){
                        sched_status = sched_cancellable_sleep_on(&mtx->km_waitq);
                        dbg(DBG_PRINT,"(GRADING1C)\n");
                }
                if(sched_status == -EINTR){
                        dbg(DBG_PRINT,"(GRADING1E)\n");
                        return -EINTR;
                }
                mtx->km_holder = curthr;
...
kmutex_unlock(kmutex_t *mtx)
{
        ...
        KASSERT(mtx != NULL && "Attempting to unlock on NULL");
        KASSERT(curthr && (curthr == mtx->km_holder));
