...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
        ...
        dbg(DBG_PRINT,"(GRADING2A)\n");
        int bytes_read = file->vn_cdev->cd_ops->read(bytedev, offset,buf,count);
        return bytes_read;
}
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
        ...
        bytedev_t *bytedev = file->vn_cdev;
        /*if(bytedev == NULL){
                dbg(DBG_PRINT,"(GRADING2X KNSI)\n");
                return -ENOTSUP;
        }*/
        KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->write);
        dbg(DBG_PRINT,"(GRADING2A)\n");
        int bytes_written = file->vn_cdev->cd_ops->write(bytedev, offset,buf,count);
...
special_file_mmap(vnode_t *file, vmarea_t *vma, mmobj_t **ret)
{
        ...
        int ret_val;
        ret_val = file->vn_cdev->cd_ops->mmap(file, vma, ret);
