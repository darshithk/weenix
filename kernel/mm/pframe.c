...
pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
        ...
        //Get pframe from resident
        pframe_t *frame = pframe_get_resident(o, pagenum);

        //If frame is not NULL, it is present in resident memory
        //Check if it is busy and sleep on waitq if busy
        //Set return pointer to frame
        if(frame != NULL){
                if(pframe_is_busy(frame)){
                        dbg(DBG_PRINT,"(GRADING3B\n");
                        sched_sleep_on(&frame->pf_waitq);
                }
                *result = frame;
        }
        //If frame is not present in resident memory
        else{
                //Allocate pframe
                frame = pframe_alloc(o, pagenum);
                dbg(DBG_PRINT,"(GRADING3B\n");
                
                //Fill the frame
                //If return code from fill is 0, fill was successful.
                //Point return to frame
                if((ret_val = pframe_fill(frame)) == 0){
                        dbg(DBG_PRINT,"(GRADING3B\n");
                        *result = frame;
                }
                //Fill failed, return error code received from fill
                else{
                        pframe_free(frame);
...
pframe_unpin(pframe_t *pf)
{
        ...
        if(pf->pf_pincount == 0){
                dbg(DBG_PRINT,"(GRADING3B\n");
                //Remove from existing list - pinned list
                list_remove(&(pf->pf_link));
