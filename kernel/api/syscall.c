...
sys_read(read_args_t *arg)
{
        ...
        ret_val = copy_from_user(&kargs, arg, sizeof(read_args_t));

        //Allocate new buffer
        buf = page_alloc();

        ret_val = do_read(kargs.fd, buf, kargs.nbytes);
        ...
        ret_val = copy_to_user(arg->buf, buf, nbytes);
...
sys_write(write_args_t *arg)
{
        ...
        //Since user called write, get contents from the user buffer
        ret_val = copy_from_user(buf, kargs.buf, kargs.nbytes);

        ret_val = do_write(kargs.fd, buf, kargs.nbytes);
...
sys_getdents(getdents_args_t *arg)
{
        ...
        int ret_val, ret_bytes_copied = 0;
        getdents_args_t kargs;
        int num_dirents, dirent_index = 0;
        dirent_t dirent;
        

        ret_val = copy_from_user(&kargs, arg, sizeof(getdents_args_t));

        num_dirents = (int)(kargs.count)/sizeof(dirent_t);

        while(num_dirents-- > 0){
                ret_val = do_getdent(kargs.fd, &dirent);

                //If there was error getting dirent
                if(ret_val < 0){
                        curthr->kt_errno = -ret_val;
                        dbg(DBG_PRINT,"(GRADING3D\n");
                        return -1;
                ...
                ret_bytes_copied += ret_val;
                ret_val = copy_to_user(kargs.dirp + dirent_index++, &dirent, ret_val);

        }
        dbg(DBG_PRINT,"(GRADING3B\n");
        return ret_bytes_copied;
}
